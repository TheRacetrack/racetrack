...
def explain_audit_log_event(ale: AuditLogEventDto) -> str:
    ...
    if ale.event_type == AuditLogEventType.JOB_DEPLOYED.value:
        return f'Job {ale.job.name} {ale.job.version} deployed by {ale.username_executor}'
        ...
        return f'Job {ale.job.name} {ale.job.version} {owner_info} redeployed by {ale.username_executor}'
    elif ale.event_type == AuditLogEventType.JOB_DELETED.value:
        owner_info = f'(owned by {ale.username_subject})' if ale.username_subject else ''
        return f'Job {ale.job.name} {ale.job.version} {owner_info} deleted by {ale.username_executor}'
    else:
