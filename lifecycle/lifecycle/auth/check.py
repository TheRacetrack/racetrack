...
def check_auth(
    ...
    if token_payload.subject_type == AuthSubjectType.INTERNAL.value:
        ...
        authorize_internal_token(token_payload, scope_str, job.name, job.version)
    else:
        if job.name:
            assert job.version, 'job.version is required when job.name is specified'
            assert scope, 'scope is required when job.name is specified'
            authorize_resource_access(auth_subject, job.name, job.version, scope.value)
        elif scope:
