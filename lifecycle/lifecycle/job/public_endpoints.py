...
def read_active_job_public_endpoints(
    ...
    """
    job_model = resolve_job_model(job.name, job.version)
    resolved_version = job_model.version

    queryset = models.PublicEndpointRequest.objects.filter(
        job__name=job.name, job__version=resolved_version, active=True
    )
...
def create_job_public_endpoint_if_not_exist(
    job.name: str, job.version: str, endpoint: str
) -> models.PublicEndpointRequest:
    job_model = read_job_model(job.name, job.version)
    try:
