...
def test_deploy_job_chain():
    ...
    _install_plugin('github.com/LookACastle/plugin-python-job-type')
    esc = _create_esc()
...
def _verify_deployed_job_chain_adder_reponse(job.name: str, auth_token: str):
    print(f'Verifying {job.name} job response...')
    pub_url = os.environ['PUB_URL']
    url = f'{pub_url}/job/{job.name}/latest/api/v1/perform'
...
def _make_wrongly_authenticated_request(job.name: str):
    print(f'Verifying requests without authentication to {job.name}...')
    pub_url = os.environ['PUB_URL']
    url = f'{pub_url}/job/{job.name}/latest/api/v1/perform'
